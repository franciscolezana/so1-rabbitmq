...
func main() {
	...
	conn, err := amqp.Dial("amqp://guest:guest@rabbitmq:5672/") //104.154.115.210
	...
	go func() {
		for d := range msgs {
			...
			req, err := http.Post("http://mongo-server:5000/", "application/json", bytes.NewBuffer(postBody)) //localhost
